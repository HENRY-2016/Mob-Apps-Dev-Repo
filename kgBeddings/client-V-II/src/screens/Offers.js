...
// APIs
import {APIOffersDetailsById,APIlistAllOffersProducts, imageUrl,} from './DataFileApis';
// import { OffersData, LoadOffersData} from './AppDataFile';
...
LoadProductsItems = () =>{
    this.LoadAppItemsData(APIlistAllOffersProducts,"OffersData")
};
...
{
    axios.get(APIOffersDetailsById+id)
    .then(res => {
        let results =JSON.stringify(res.data);
        let jsonResults =JSON.parse(results); 
        let imageSlider = [];
        ...
        <View style={styles.homeCardView2}>
            ...
            <View style={styles.homeOrderBtnView}>
                <View style={[styles.centerElement, styles.homeOrderBtn]}>
                    <TouchableOpacity style={styles.homeOrdersBtn} onPress={()=>addItemsToCart(index,this.state.OffersData)} >
...
render() {
    const {IsDeviceConnected,OffersData,NumberOfItems,DetailsName,DetailsShortText,DetailsAmount,DetailsLongText} = this.state;
    ...
    return (
        ...
        {IsDeviceConnected ?(<>
            {DoNotShowDisplayScreen ? <></> :(<>
                    <FlatList
                    data={ formatData(OffersData,numColumns)}
            ...
            {DoNotShowItemDetailsScreen ? <></>:(<>
                <ScrollView showsVerticalScrollIndicator={false} >
                    {renderItemDetailsViewUi (ItemDetails,DetailsName,DetailsShortText,DetailsLongText,DetailsAmount,
                        this.showItemDisplayScreen,ItemIndex,OffersData)}
